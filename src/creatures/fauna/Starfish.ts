 ...
 * than to a brittle star, which is what "storybook" asks for. The dome falls
 * off on a rounded curve so the silhouette has a shoulder rather than a cone.
 * Vertex colour lightens toward the rim: a marking (arm tips are pale on the
 * real animal), deliberately mild so it does not fight the ramp's own shading.
 */
function createStarGeometry(): BufferGeometry {
  ...
  for (let ring = 1; ring <= RINGS; ring++) {
    ...
    for (let j = 0; j < SEGMENTS; j++) {
      ...
      const lobe = Math.pow(0.5 + 0.5 * Math.cos(5 * theta), 1.5);
      const arm = 0.55 + 0.45 * lobe;
      const r = t * arm;
      const y = DOME * Math.pow(Math.max(0, 1 - t * t), 0.6);
