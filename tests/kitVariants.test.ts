...
describe("kit variants: placements byte-unchanged vs the pre-change build", () => {
  ...
  for (const key of Object.keys(fixture.regions)) {
    it(`keeps every ${key} region placement byte-identical`, () => {
      ...
      // The fence is APPEND-ONLY, not frozen: later waves (roads-and-
      // axes' fresh-substream servings) may add nodes AFTER the recorded
      // truth — region builds mount new modules last, so the walk keeps
      // every pre-change node at its recorded index. Every recorded node
      // must still stand, byte-identical, in order; only net-new nodes
      // may follow.
